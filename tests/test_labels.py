...
def test_a_sheet_is_a_pdf():
    pdf = labels.sheet(CODES[:1], "10th Richmond", "https://example.org/g")
...
def test_thirty_three_codes_need_a_second_page():
    one = labels.sheet(CODES[:32], "10th Richmond", "https://example.org/g")
    two = labels.sheet(CODES[:33], "10th Richmond", "https://example.org/g")
...
def test_the_qr_url_is_base_slash_code():
    ...
    with_slash = labels.sheet(CODES[:1], "10th Richmond", "https://example.org/g/")
    without = labels.sheet(CODES[:1], "10th Richmond", "https://example.org/g")
