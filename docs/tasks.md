...
- [ ] Notifications by email: each user ticks categories on their own settings page. Categories: a found report, a new
      repair ticket. Sent through the Admin's SMTP account (FR-USR-15); nothing is sent when none is set. One mail per
      event, no digest. New FR row; NFR-SEC-05 is unchanged because the preference is a setting, not a field about the
      person.
