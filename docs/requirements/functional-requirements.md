...
| FR-INV-08 | Must     | Filter the list by location, sub-location, and status (in, out, reserved, in repair, missing). Category joins the filters when it is built.                                                                                |
...
| FR-INV-23 | Must     | A new unit is offered the next free number, editable before save, because a number may already be written on the gear. Numbers are unique within a generic. A unit may also carry a nickname.                              |
...
| FR-INV-26 | Must     | Marking a single item as generic does not change that item. A new generic takes its name, and the item becomes unit #1 under it, keeping its code and history.                                                             |
