...
| FR-SET-03 | Must     | An item's home has an optional free-form **shelf** label, e.g. "shelf 4" or "trailer 2". No fixed list, no hierarchy, no referential integrity. It is a label, not an entity. Stored as `sub_location`. |
| FR-SET-04 | Must     | A shelf label cannot be renamed in one place, because it is not stored in one place. Changing several items means editing those items.                                                                  |
| FR-SET-05 | Must     | Deleting a location still in use is blocked. The message names the items blocking it. The same rule applies to categories (FR-SET-07).                                                                  |
| FR-SET-10 | Won't    | Withdrawn: types as a list in Settings. A generic item and its units (FR-INV-21) carry what a type carried, and live in the inventory.                                                                  |
...
| FR-SET-09 | Won't    | Withdrawn: nested shelves. Free-form labels do not nest.                                                                                                                                                |
...
| ID        | Priority | Requirement                                                                                                                                                                                                                                                                                       |
| --------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| FR-INV-01 | Must     | Create, view, edit, and retire a gear item. Required fields: name, home location, home shelf, notes. Category (FR-SET-07) appears when it is built.                                                                                                                                               |
...
| FR-INV-08 | Must     | Filter the list by location, shelf, and status (in, out, reserved, in repair, missing). Category joins the filters when it is built.                                                                                                                                                              |
| FR-INV-09 | Must     | The item page shows status and holder, home, access history, repair history, and upcoming reservations.                                                                                                                                                                                           |
| FR-INV-21 | Must     | An item can be **generic**: one thing the group owns several of, e.g. "4-person tent, Brand X". It carries the name, description, photos, and purchase details once. It has no code and cannot be checked out.                                                                                    |
| FR-INV-22 | Must     | A generic item has **units**. Each unit has its own code, home, movements, repairs, missing and retired state. A unit is shown as the generic's name plus its number, e.g. "4-person tent, Brand X #3".                                                                                           |
| FR-INV-23 | Must     | A unit's number is text, unique within its generic, because what is written on the gear may be "A" or "3b". A new unit is offered the next free number after the largest numeric one, editable before save. A unit may also carry a nickname.                                                     |
| FR-INV-24 | Must     | Scanning an unassigned code offers "another of" a recent generic as well as a new item (FR-TAG-07). One tap makes the unit, with the generic's default home.                                                                                                                                      |
| FR-INV-25 | Must     | The list shows one row per generic with counts of units in and out, opening to the units. Search matches the generic's name. Filters apply to units and show the generics that have any.                                                                                                          |
| FR-INV-26 | Must     | Marking a single item as generic does not change that item. A new generic takes its name, and the item becomes a unit under it with a number the person confirms (offered "1"), keeping its code and history.                                                                                     |
| FR-INV-27 | Must     | A generic can be retired only when every unit is retired. Retiring the last unit does not retire the generic.                                                                                                                                                                                     |
| FR-INV-30 | Must     | Group two single items as units of one generic: the generic takes the chosen item's name, both become units with numbers the person confirms. Grouping with an existing generic or unit adds this item to that generic. Distinct from merging (FR-INV-13), which is for one thing recorded twice. |
| FR-INV-31 | Should   | An item's history and a repair list show the full record from the server when online. Offline, they show what the device holds (NFR-DATA-03) and say so.                                                                                                                                          |
...
| FR-INV-10 | Should   | Browse by location then shelf, to answer "what belongs on shelf 4?".                                                                                                                                                                                                                              |
...
| FR-RES-16 | Must     | Any signed-in user can create, edit, and cancel any reservation. A reservation has no owner; the group plans as one team.                                                                                                                                                          |
...
| FR-RES-07 | Should   | Scanning an item that is not on the reservation appends it to the check-out and adds it to the reservation's gear list. No warning, no extra tap. If the unit's generic is on the list and that line is full, the quantity rises by one instead.                                   |
...
| FR-RES-13 | Should   | Reserve a quantity of a generic item ("4 × 4-person tent, Brand X") instead of naming units (FR-INV-21). At check-out, scanning any of its units ticks one off (FR-RES-02).                                                                                                        |
| FR-RES-15 | Should   | A reservation of a generic item conflicts (FR-RES-05) when the quantity reserved across overlapping dates exceeds its unretired units.                                                                                                                                             |
| FR-RES-17 | Should   | Link an item that is already out to a reservation, from the reservation page. The movement's event is corrected by an appended record (as FR-OUT-16), and the item joins the gear list. Fixes gear taken before the reservation existed, or under no event.                        |
...
| FR-USR-13 | Must     | The first Admin account is created from the command line or the seed file (NFR-DEP-10) when the server is installed. There is no open sign-up.                                                                                                  |
...
## 11. Assistant access (MCP)

| ID        | Priority | Requirement                                                                                                                                                                                                                                                      |
| --------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| FR-MCP-01 | Should   | The server is an MCP server. A signed-in user connects an assistant with a token they mint themselves in Settings. No Admin is involved.                                                                                                                         |
| FR-MCP-02 | Should   | The token is a device session (FR-USR-07). It is listed with the user's phones and revoked the same way (FR-USR-14).                                                                                                                                             |
| FR-MCP-03 | Should   | Tools cover what a User can do in the app: read and search items, what is out, reservations, repairs; create, edit, and cancel reservations; create and edit items, mark missing; raise and comment on tickets; check out and in by item (FR-OUT-02, FR-OUT-07). |
| FR-MCP-04 | Should   | Nothing an Admin does is reachable: users, mail, settings, locations, printing codes.                                                                                                                                                                            |
