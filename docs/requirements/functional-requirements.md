...
| FR-SET-10 | Should   | The Quartermaster can define **types**, e.g. "4-person tent, Brand X" or "2-burner propane stove", and assign items to them. Items of one type are interchangeable for reservations (FR-RES-13). Nothing else is shared: condition, repairs, and history stay per item (FR-INV-06, FR-REP-01). |
...
| FR-INV-08 | Must     | Filter the list by location, sub-location, and status (in, out, reserved, in repair, missing). Type, category, and condition join the filters when they are built.                                                         |
