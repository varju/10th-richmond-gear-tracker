...
| NFR-DATA-11 | Must     | The app asks the browser for persistent storage. A refusal is not shown; the unsent count (FR-OFF-04) is the warning, because iOS refuses every site and the user's action is the same.                                                      |
...
| NFR-SEC-03 | Must     | Public item pages expose only the group name and a contact route, never the item's name. No personal data, no prices, no history.                                                                           |
...
| NFR-SEC-10 | Must     | The mail account's password (FR-USR-15) stays on the server. It is never in the event log and never on a device.                                                                                            |
