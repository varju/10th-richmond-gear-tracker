...
How well the system must work. Drafts for review; several encode open choices. See
...
An iPad left in the locker is B with sync off, so B covers it. But two of three locations are unheated or outdoors, so a
stationed tablet is a weak fallback. Design for the phones people carry.
...
We hold names and email addresses. Scouts Canada and parents both have expectations about youth data.
